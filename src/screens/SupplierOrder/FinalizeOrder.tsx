...
export default function FinalizeOrderScreen({
  ...
  const [{loading: fetchDeliveryDates}, generateReasonableDeliveryTime] =
    useMutation({
      url: 'delivery-dates',
    });
  ...
  return (
    <>
      {reasonableDeliveryTime.length === 0 ? (
        <Loading />
      ) : (
        <Container className="">
          <ScrollView
            ...
            <View className="bg-primary p-2.5 mt-3">
              <Text className="text-white text-30 font-bold">
                {supplier?.name}
              </Text>
            </View>
            ...
            <FormGroup>
              ...
              <CalendarInput
                ...
                maximumDate={dayjs(
                  _.get(
                    reasonableDeliveryTime[reasonableDeliveryTime.length - 1],
                    'value',
                  ),
                ).format('YYYY-MM-DD')}
                disableElementValues={selectDisableDateIndex({
                  days: reasonableDeliveryTime,
                })}
